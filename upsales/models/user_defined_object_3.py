"""User-defined object 3 models for Upsales API.

This module defines models for user-defined custom objects (slot 3).
"""
...
if TYPE_CHECKING:
    from upsales.types import UserDefinedObject3UpdateFields


class UserDefinedObject3(BaseModel):
    """User-defined object 3 model.

    Represents a flexible custom entity with associations to clients, contacts, projects,
    users, and roles (third slot).
    ...
    async def edit(self, **kwargs: Unpack[UserDefinedObject3UpdateFields]) -> UserDefinedObject3:
        ...
        Returns:
            Updated UserDefinedObject3 instance.
        ...
        return await self._client.user_defined_object_3.update(
            self.id, **self.to_api_dict(**kwargs)
        )


class PartialUserDefinedObject3(PartialModel):
    """Partial user-defined object 3 model for nested responses.
    ...
    Example:
        ...
        partial: PartialUserDefinedObject3 = some_object.user_defined_object
        full: UserDefinedObject3 = await partial.fetch_full()
    ...
    async def fetch_full(self) -> UserDefinedObject3:
        """Fetch complete user-defined object data.

        Returns:
            Full UserDefinedObject3 instance.
        ...
        Example:
            ```python
            partial = PartialUserDefinedObject3(id=1, notes="Notes")
        ...
        if not self._client:
            raise RuntimeError("No client available")
        return await self._client.user_defined_object_3.get(self.id)
    ...
    async def edit(self, **kwargs: Unpack[UserDefinedObject3UpdateFields]) -> UserDefinedObject3:
        ...
        if not self._client:
            raise RuntimeError("No client available")
        return await self._client.user_defined_object_3.update(self.id, **kwargs)
